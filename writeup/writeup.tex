...
\usepackage{booktabs, hyperref, listings, graphicx, float, parskip}
...
Our methodology utilizes a pipeline that ingests raw station data, generates astronomically derived clear-sky expectations, and augments the training set with realistic, synthetic sensor errors. These engineered features are processed by both the Random Forest and Isolation Forest models, whose outputs are then combined with the original feature set and passed to the RNN to produce final quality probability scores. Finally, a manual quality-control application enables a human-in-the-loop review and model-training system that maintains expert oversight while iteratively improving the model's predictive power.
...
The features engineered in preprocessing to support the model with more meaningful and physically interpretable information are summarized in Table~\ref{tab:engineered_features}.

\begin{table}[H]
    \centering
    \caption{Engineered Features Summary}\label{tab:engineered_features}
    \begin{tabular}{lcp{9cm}}
        ...
        & CSI & Clear-sky index (ratio of measured GHI to GHI\_Clear, clipped to [0,3] to prevent instability from extreme values). \\
        \midrule
...
The ROC curves for each target are depicted in Figure~\ref{fig:roc_curves}, illustrating the trade-off between true positive rates and false positive rates. The curves clearly illustrate that the Global Horizontal Irradiance (GHI) model exhibits the strongest bow toward the top left, achieving the highest Area Under the Curve (AUC) across both datasets (0.991 for regular data, and 0.985 for error-injected data). Diffuse Horizontal Irradiance (DHI) follows closely behind (0.990 for the regular data, and 0.955 for the error-injected data), while Direct Normal Irradiance (DNI), yields a noticeably shallower curve (AUC of 0.956 for the regular data, and 0.922 for the error-injected data). This reflects the DNI model's struggle to separate transient noise from true tracking anomalies. Furthermore, comparing the regular files with the injected-error files reveals slightly higher AUC values for the regular data. This likely reflects differences in class separability between real and synthetic anomalies, rather than a direct indication of improved sensitivity to real-world faults.
...
Additionally, ambiguity in human labeling for DNI anomalies introduces noise into the ground truth, further limiting achievable model performance. Because multiple irradiance features can plausibly explain a given anomaly, inconsistencies in assigning a single ``responsible'' feature reduce the reliability of supervised learning signals for this target.

Meanwhile, anomalous DHI values are more challenging to identify because of their comparatively low magnitudes and the difficulty of translating them into obvious anomaly-signaling features. This leads the DHI model to tend towards identifying more false positives in the regular data. Because of this, the model is designed to take a more conservative approach, erring on the side of caution when flagging points as anomalies. This behavior differs between the regular and error-injected datasets, with the model predicting more false positives on the regular data and more false negatives on the error-injected data. 
...
It is worth noting that although the model's performance, as measured by traditional metrics, is insufficient for unsupervised deployment, its practical utility in a human-in-the-loop context remains significant. Because of this, it is better to view the model as a decision-support tool rather than a fully autonomous classifier. While the model does not always capture the full temporal range or correct causal features of an anomalous event, a significant fraction of events, marked by one or more ``bad'' points, are at least partially identified. In Figures~\ref{fig:focus_window_01} and \ref{fig:focus_window_06}, we can see two examples of such days, comparing model predictions to human labels.
...
This project successfully developed and deployed a hybrid machine-learning quality-control pipeline for the University of Oregon's Solar Radiation Monitoring Lab. By integrating a Random Forest, an Isolation Forest, and an Attention-based RNN, the project moved beyond traditional rule-based flagging to identify complex, time-series irradiance anomalies. While automated flagging of highly sensitive metrics such as DNI remains challenging, the system achieved high precision and strong overall accuracy. Ultimately, by optimizing for recall and wrapping the models in a custom analyst GUI, the system achieved its primary goal: substantially reducing the manual burden on researchers while ensuring expert oversight remains central to the SRML's data validation process.

\section*{Code Availability}
The source code for the full machine-learning quality-control pipeline, including data preprocessing, synthetic error injection, model training, inference, and analysis scripts, is publicly available at GitHub \cite{chester2026srmlmlqc}.
...
\bibitem{chester2026srmlmlqc}
Chester, K. (2026). \textit{srml-ml-qc} [Computer software]. GitHub. \url{https://github.com/KalelChester/srml-ml-qc}
