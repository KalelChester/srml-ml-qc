...
To extract meaningful signals for the machine learning models, a custom data pipeline was engineered. The pipeline utilizes physics libraries such as \texttt{pvlib} to calculate clear-sky irradiance models for the Seattle site. These physically derived expectations serve as baseline comparisons for the actual sensor readings, allowing the models to evaluate irradiance deviations dynamically without arbitrarily altering the native timestamps.
...
The core of the QC system is a three-pronged ensemble architecture built in python, leveraging \texttt{scikit-learn} and \texttt{JAX/Flax}.
